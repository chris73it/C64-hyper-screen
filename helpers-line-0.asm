...
.macro stabilize_irq() {
  ...
  !loop: // (2+3) * 7
    ...
    bne !loop- // 2
    //bit $ea
    nops(2) // We wait 4 cycles instead than 3 because line 0 increases
            // raster register one cycle later than all other lines.
  test:
    lda vic2_rasterline_register
    cmp vic2_rasterline_register
    beq next_instruction  // 2 or 3 depending...
  next_instruction:
}
